...
static int decodeGameRotationVector(sh2_SensorValue_t *value, const sh2_SensorEvent_t *event)
{
    ...
    int16_t raw_i = read16(&event->report[4]);
    int16_t raw_j = read16(&event->report[6]);
    int16_t raw_k = read16(&event->report[8]);
    int16_t raw_real = read16(&event->report[10]);

    // Scale the raw values to obtain floating-point quaternion components
    float scaled_i = raw_i * SCALE_Q(14);
    float scaled_j = raw_j * SCALE_Q(14);
    float scaled_k = raw_k * SCALE_Q(14);
    float scaled_real = raw_real * SCALE_Q(14);

    // Assign the scaled values to the sensor value structure
    value->un.gameRotationVector.i = scaled_i;
