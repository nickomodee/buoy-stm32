...
class Buoy {
    ...
    private:
        static constexpr uint8_t WAKE_PERIOD = 15;
        static constexpr uint8_t MINUTES_BEFORE_ALARM_THRESHOLD = 1;
        ...
        static constexpr size_t DATA_BUFFER_SIZE_ = firmware_version_type_size + status_type_size + (status_type_size + second_type_size + minute_type_size + hour_type_size + day_type_size + dayofweek_type_size + month_type_size + year_type_size) + ((sizeof(num_temp_sensors) + (status_type_size + temp_sensor_type_size) * num_temp_sensors) + (sizeof(num_humidity_sensors)  +  (status_type_size + humidity_sensor_type_size) * num_humidity_sensors) + (sizeof(num_pressure_sensors)  +  (status_type_size + pressure_sensor_type_size) * num_pressure_sensors) + (sizeof(num_uv_sensors) + (status_type_size + uv_sensor_type_size) * num_uv_sensors)  +  (sizeof(num_visible_light_sensors)   +   (status_type_size + visible_light_sensor_type_size) * num_visible_light_sensors) + (sizeof(num_ir_light_sensors)   +   (status_type_size + ir_light_sensor_type_size) * num_ir_light_sensors));
        uint8_t data_buffer_[DATA_BUFFER_SIZE_]; //     [Firmware Version]   ->    [SD Status]   ->    [RTC Status]   ->    [Second]    ->     [Minute]    ->     [Hour]    ->     [Day]    ->    [Day of Week]    ->    [Month]    ->     [Year]     ->      [Num Temp Sensors] -> [Temp Sensor 1 Status] -> [Temp Sensor 1 Data]  ->   ...      ->       [Num Humidity Sensors] -> [Humidity Sensor 1 Status] -> [Humidity Sensor 1 Data]  ->   ...       ->        [Num Pressure Sensors] -> [Pressure Sensor 1 Status] -> [Pressure Sensor 1 Data]  ->   ...       ->        [Num UV Sensors] ->  [UV Sensor 1 Status] -> [UV Sensor 1 Data]  ->   ...       ->       [Num Visible Light Sensors] -> [Visible Light Sensor 1 Status] -> [Visible Light Sensor 1 Data]  ->   ...        ->         [Num IR Light Sensors]  ->  [IR Light Sensor 1 Status] -> [IR Light Sensor 1 Data]  ->   ...
...
template<const uint8_t num_temp_sensors, const uint8_t num_humidity_sensors, const uint8_t num_pressure_sensors, const uint8_t num_uv_sensors, const uint8_t num_visible_light_sensors, const uint8_t num_ir_light_sensors>
Buoy<num_temp_sensors, num_humidity_sensors, num_pressure_sensors, num_uv_sensors, num_visible_light_sensors, num_ir_light_sensors>::Buoy(const uint8_t init_retries, const firmware_version_type firmware_version, DataSender* data_sender, const std::array<TempSensor*, num_temp_sensors>& temp_sensors_array, const std::array<HumiditySensor*, num_humidity_sensors>& humidity_sensors_array, const std::array<PressureSensor*, num_pressure_sensors>& pressure_sensors_array, const std::array<UVSensor*, num_uv_sensors>& uv_sensors_array, const std::array<VisibleLightSensor*, num_visible_light_sensors>& visible_light_sensors_array, const std::array<IRLightSensor*, num_ir_light_sensors>& ir_light_sensors_array) : init_retries_(init_retries), firmware_version_(firmware_version), data_sender_(data_sender), temp_sensors_array_(temp_sensors_array), humidity_sensors_array_(humidity_sensors_array), pressure_sensors_array_(pressure_sensors_array), uv_sensors_array_(uv_sensors_array), visible_light_sensors_array_(visible_light_sensors_array), ir_light_sensors_array_(ir_light_sensors_array) {}
...
void Buoy<num_temp_sensors, num_humidity_sensors, num_pressure_sensors, num_uv_sensors, num_visible_light_sensors, num_ir_light_sensors>::prepare_for_sleep() {
    // put any sensors to sleep, like imu when implemented
    const uint8_t minute_to_wake = calculate_minute_to_wake_();
    sleep.prepare_for_sleep(0, minute_to_wake, 0, true, false); // first `0` indicates the second because we want it on the minute, second `0` because we are masking the hour so the value doesn't matter, `true` because we are masking the hour as we only care about the minute and second, `false` because we don't want to mask the minute
}
...
uint8_t Buoy<num_temp_sensors, num_humidity_sensors, num_pressure_sensors, num_uv_sensors, num_visible_light_sensors, num_ir_light_sensors>::calculate_minute_to_wake_() {
    ...
    const uint8_t time_to_next_interval = current_minute % WAKE_PERIOD;
    const uint8_t num_intervals_to_wait = ((WAKE_PERIOD - time_to_next_interval) > MINUTES_BEFORE_ALARM_THRESHOLD) ? 1 : 2;
    const uint8_t next_interval = (current_minute / WAKE_PERIOD) + num_intervals_to_wait;
    const uint8_t minute_to_wake = (next_interval * WAKE_PERIOD) % 60;
...
bool Buoy<num_temp_sensors, num_humidity_sensors, num_pressure_sensors, num_uv_sensors, num_visible_light_sensors, num_ir_light_sensors>::send_data() {
    return data_sender_->send((const char*)data_buffer_, DATA_BUFFER_SIZE_);
}