...
size_t PAL_STM32_WIRE::requestFrom(const uint8_t address, size_t size) {
    ...
    uint8_t* read_ptr = const_cast<uint8_t*>(this->rx_buffer_.get_read_ptr()); // I mean this isn't good, but I'd rather use the memory used for the rx buffer in base `Stream` instead of just using an extra array
    HAL_I2C_Master_Receive(PAL_STM32_WIRE::get_hi2c_ptr(), PAL_STM32_WIRE::tx_address_, read_ptr, size, WIRE_MAX_TIMEOUT);
    this->rx_buffer_.set_count(size);
    return size;
}
