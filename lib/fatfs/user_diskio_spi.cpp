...
static volatile
DSTATUS __FIRMWARE_DATA Stat = STA_NOINIT;	/* Physical drive status */


static
BYTE __FIRMWARE_BSS CardType;			/* Card type flags */

uint32_t __FIRMWARE_BSS spiTimerTickStart;
uint32_t __FIRMWARE_BSS spiTimerTickDelay;

void __FIRMWARE SPI_Timer_On(uint32_t waitTicks) {
    spiTimerTickStart = HAL_GetTick();
    spiTimerTickDelay = waitTicks;
}

uint8_t __FIRMWARE SPI_Timer_Status() {
...
static
void __FIRMWARE rcvr_spi_multi (
...
/* Send multiple byte */
static
void __FIRMWARE xmit_spi_multi (
	const BYTE *buff,	/* Pointer to the data */
	UINT btx			/* Number of bytes to send (even number) */
)
{
	HAL_SPI_Transmit(&SD_SPI_HANDLE, (uint8_t*)buff, btx, 10000);
}
#endif
...
static
int __FIRMWARE wait_ready (	/* 1:Ready, 0:Timeout */
	UINT wt			/* Timeout [ms] */
)
{
	BYTE d;
...
static
void __FIRMWARE despiselect (void)
{
	CS_HIGH();		/* Set CS# high */
...
static
int __FIRMWARE spiselect (void)	/* 1:OK, 0:Timeout */
{
	CS_LOW();		/* Set CS# low */
...
static
int __FIRMWARE rcvr_datablock (	/* 1:OK, 0:Error */
...
#if _USE_WRITE
static
int __FIRMWARE xmit_datablock (	/* 1:OK, 0:Failed */
...
static
BYTE __FIRMWARE send_cmd (		/* Return value: R1 resp (bit7==1:Failed to send) */
...
//The following functions are defined as because they aren't the functions that
...
DSTATUS __FIRMWARE USER_SPI_initialize (
...
DSTATUS __FIRMWARE USER_SPI_status (
	BYTE drv		/* Physical drive number (0) */
)
{
...
DRESULT __FIRMWARE USER_SPI_read (
	BYTE drv,		/* Physical drive number (0) */
...
#if _USE_WRITE
DRESULT __FIRMWARE USER_SPI_write (
	BYTE drv,			/* Physical drive number (0) */
...
#if _USE_IOCTL
DRESULT __FIRMWARE USER_SPI_ioctl (
	BYTE drv,		/* Physical drive number (0) */
