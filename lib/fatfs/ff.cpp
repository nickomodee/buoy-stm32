...
#endif
static FATFS* __FIRMWARE_BSS FatFs[_VOLUMES];	/* Pointer to the file system objects (logical drives) */
static WORD __FIRMWARE_BSS Fsid;				/* File system mount ID */

#if _FS_RPATH && _VOLUMES >= 2
static BYTE __FIRMWARE_BSS CurrVol;			/* Current drive */
#endif

#if _FS_LOCK
static FILESEM __FIRMWARE_BSS Files[_FS_LOCK];	/* Open object lock semaphores */
#endif
...
static WCHAR __FIRMWARE_BSS LfnBuf[_MAX_LFN + 1];
#define	DEFINE_NAMEBUF		BYTE sfn[12]
...
#ifdef _EXCVT
static const BYTE __FIRMWARE_RODATA ExCvt[] = _EXCVT;	/* Upper conversion table for extended characters */
#endif
...
---------------------------------------------------------------------------*/
DWORD clust2sect (FATFS* fs, DWORD clst) __FIRMWARE;
DWORD get_fat (FATFS* fs,	DWORD clst) __FIRMWARE;

#if !_FS_READONLY
FRESULT put_fat (FATFS* fs,	DWORD clst,	DWORD val) __FIRMWARE;
#endif /* !_FS_READONLY */

#if _USE_LFN
static void gen_numname (BYTE* dst, const BYTE* src, const WCHAR* lfn, UINT seq) __FIRMWARE;
#endif /* !_USE_LFN */
...
static
void __FIRMWARE mem_cpy (void* dst, const void* src, UINT cnt) {
	BYTE *d = (BYTE*)dst;
...
/* Fill memory */
static
void __FIRMWARE mem_set (void* dst, int val, UINT cnt) {
	BYTE *d = (BYTE*)dst;
...
static
int __FIRMWARE mem_cmp (const void* dst, const void* src, UINT cnt) {
...
static
int __FIRMWARE chk_chr (const char* str, int chr) {
...
static
FRESULT __FIRMWARE chk_lock (	/* Check if the file can be accessed */
...
static
int __FIRMWARE enq_lock (void)	/* Check if an entry is available for a new object */
{
	UINT i;
...
static
UINT __FIRMWARE inc_lock (	/* Increment object open counter and returns its index (0:Internal error) */
...
static
FRESULT __FIRMWARE dec_lock (	/* Decrement object open counter */
...
static
void __FIRMWARE clear_lock (	/* Clear lock entries of the volume */
	FATFS *fs
)
{
	UINT i;
...
#if !_FS_READONLY
static
FRESULT __FIRMWARE sync_window (
	FATFS* fs		/* File system object */
)
{
	DWORD wsect;
...
static
FRESULT __FIRMWARE move_window (
	FATFS* fs,		/* File system object */
...
#if !_FS_READONLY
static
FRESULT __FIRMWARE sync_fs (	/* FR_OK: successful, FR_DISK_ERR: failed */
	FATFS* fs		/* File system object */
)
{
	FRESULT res;
...
DWORD __FIRMWARE clust2sect (	/* !=0: Sector number, 0: Failed - invalid cluster# */
	FATFS* fs,		/* File system object */
...
DWORD __FIRMWARE get_fat (	/* 0xFFFFFFFF:Disk error, 1:Internal error, 2..0x0FFFFFFF:Cluster status */
	FATFS* fs,	/* File system object */
...
#if !_FS_READONLY
static
FRESULT __FIRMWARE remove_chain (
	FATFS* fs,			/* File system object */
...
#if !_FS_READONLY
static
DWORD __FIRMWARE create_chain (	/* 0:No free cluster, 1:Internal error, 0xFFFFFFFF:Disk error, >=2:New cluster# */
	FATFS* fs,			/* File system object */
...
#if _USE_FASTSEEK
static
DWORD __FIRMWARE clmt_clust (	/* <2:Error, >=2:Cluster number */
...
static
FRESULT __FIRMWARE dir_next (	/* FR_OK:Succeeded, FR_NO_FILE:End of table, FR_DENIED:Could not stretch */
...
#if _USE_LFN
static
const BYTE __FIRMWARE_RODATA LfnOfs[] = {1,3,5,7,9,14,16,18,20,22,24,28,30};	/* Offset of LFN characters in the directory entry */


static
int __FIRMWARE cmp_lfn (			/* 1:Matched, 0:Not matched */
...
static
int __FIRMWARE pick_lfn (			/* 1:Succeeded, 0:Buffer overflow */
...
#if !_FS_READONLY
static
FRESULT __FIRMWARE dir_register (	/* FR_OK:Successful, FR_DENIED:No free entry or too many SFN collision, FR_DISK_ERR:Disk error */
...
static
FRESULT __FIRMWARE dir_remove (	/* FR_OK: Successful, FR_DISK_ERR: A disk error */
...
static
void __FIRMWARE get_fileinfo (		/* No return code */
	DIR* dp,			/* Pointer to the directory object */
...
static
WCHAR __FIRMWARE get_achar (		/* Get a character and advances ptr 1 or 2 */
...
static
int __FIRMWARE pattern_matching (	/* Return value: 0:mismatched, 1:matched */
...
static
FRESULT __FIRMWARE create_name (
	DIR* dp,			/* Pointer to the directory object */
...
#endif
		if (w < 0x80 && chk_chr(firmware_illegal_chars, w)) /* Reject illegal characters for LFN */
			return FR_INVALID_NAME;
	...
	*path = &p[si];						/* Return pointer to the next segment */
	...
	for (;;) {
		...
		if (_DF1S && w >= 0x100) {		/* DBC (always false at SBCS cfg) */
			...
			if (!w || chk_chr(firmware_illegal_chars_2, w)) {	/* Replace illegal characters for SFN */
	...
	for (;;) {
		...
		if (IsDBCS1(c)) {				/* Check if it is a DBC 1st byte (always false on SBCS cfg) */
			...
			if (chk_chr(firmware_illegal_chars, c))	/* Reject illegal chrs for SFN */
				return FR_INVALID_NAME;
...
static
FRESULT __FIRMWARE follow_path (	/* FR_OK(0): successful, !=0: error code */
...
static
int __FIRMWARE get_ldnumber (		/* Returns logical drive number (-1:invalid drive) */
...
)
{
	...
	static const char* const __FIRMWARE_RODATA str[] = {_VOLUME_STRS};
	const char *sp;
	char c;
	TCHAR tc;
#endif
...
static
BYTE __FIRMWARE check_fs (	/* 0:FAT boor sector, 1:Valid boor sector but not FAT, 2:Not a boot sector, 3:Disk error */
	FATFS* fs,	/* File system object */
...
static
FRESULT __FIRMWARE find_volume (	/* FR_OK(0): successful, !=0: any error occurred */
...
static
FRESULT __FIRMWARE validate (	/* FR_OK(0): The object is valid, !=0: Invalid */
...
FRESULT __FIRMWARE f_findfirst (
...
#endif
#endif
			if (!w || chk_chr(firmware_illegal_chars, w) || j >= (UINT)((w >= 0x100) ? 10 : 11)) /* Reject invalid characters for volume label */
...
FRESULT __FIRMWARE f_mkfs (
	const TCHAR* path,	/* Logical drive number */
	BYTE sfd,			/* Partitioning rule 0:FDISK, 1:SFD */
	UINT au				/* Size of allocation unit in unit of byte or sector */
)
{
	static const WORD __FIRMWARE_RODATA vst[] = { 1024,   512,  256,  128,   64,    32,   16,    8,    4,    2,   0};
	static const WORD __FIRMWARE_RODATA cst[] = {32768, 16384, 8192, 4096, 2048, 16384, 8192, 4096, 2048, 1024, 512};
	int vol;
	...
	mem_set(tbl, 0, SS(fs));
	mem_cpy(tbl, firmware_boot_jump_code_oem_name, 11);/* Boot jump code, OEM name */
	i = SS(fs);								/* Sector size */
	...
	if (fmt == FS_FAT32) {
		...
		mem_cpy(tbl + BS_VolLab32, firmware_volume_label_fat32_signature, 19);	/* Volume label, FAT signature */
	} else {
		ST_DWORD(tbl + BS_VolID, n);		/* VSN */
		ST_WORD(tbl + BPB_FATSz16, n_fat);	/* Number of sectors per FAT */
		tbl[BS_DrvNum] = 0x80;				/* Drive number */
		tbl[BS_BootSig] = 0x29;				/* Extended boot signature */
		mem_cpy(tbl + BS_VolLab, firmware_volume_label_fat_signature, 19);	/* Volume label, FAT signature */
	}
