...
MAX_PACKET_SIZE: Literal[53] = 53
PACKET_OVERHEAD: Literal[5] = 5  # the packet 'overhead' that isn't encrypted data
MAX_MESSAGE_SIZE: Literal[47] = 47
MESSAGE_OVERHEAD: Literal[7] = 7 # the message 'overhead' that isn't data
...
MESSAGE_CHECKSUM_INDEX: Literal[5] = 5
MESSAGE_DATA_SIZE_INDEX: Literal[6] = 6
MESSAGE_DATA_START_INDEX: Literal[7] = 7
...
PACKET_CHECKSUM_INDEX: Literal[4] = 4
PACKET_ENCRYPTED_START_INDEX: Literal[5] = 5
...
class Packet:
    ...
    def generate_message_checksum(self) -> int:
        checksum: int = 0
        checksum += self._type.value
        checksum += self._index & 0xFF
        checksum += (self._index >> 8) & 0xFF
        checksum += (self._index >> 16) & 0xFF
        checksum += (self._index >> 24) & 0xFF
        # skip checksum for calculating the checksum
        checksum += len(self._data)
        DATA_SIZE: int = min(len(self._data), MAX_DATA_SIZE)
        for i in range(DATA_SIZE):
            checksum += int(self._data[i])
        checksum %= 0x100
        return checksum
    ...
    def generate_packet_checksum(self) -> int:
        ...
        for i in range(PACKET_SIZE):
            if (i == PACKET_CHECKSUM_INDEX) or (i == PACKET_ILLEGAL_CHAR_INDEX): # skip checksum and illegal char replacement (since if the checksum has an illegal char, we still want to handle this) for calculating the checksum
                continue
            checksum += int(self._packet[i])
        checksum %= 0x100
        return checksum
    ...
    def from_raw(self) -> bool:
        ...
        self.set_packet_checksum(self._packet[PACKET_CHECKSUM_INDEX])
        ...
        self.set_message_checksum(message[MESSAGE_CHECKSUM_INDEX])
    ...
    def generate_raw(self) -> None:
        ...
        self._packet = replace_byte_at_index(self._packet, PACKET_CHECKSUM_INDEX, PACKET_CHECKSUM) # this is annoying maybe switch all bytes to bytearray to allow indexed assignment?
...
if __name__ == "__main__":
    ...
    if not decoded_packet.from_raw():
        print("ERROR: Packet decode failed!")
